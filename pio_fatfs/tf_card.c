...
{
    // Domeu, we will not change the clock speed
...
{
    cs_deselect(SPI_SD_CSN_PIN);
}

static void CS_LOW(void)
{
    cs_select(SPI_SD_CSN_PIN);
}
...
void init_spi(void)
{
	...
	/* DOMEU:  we will rely on the PIO SPI initialisation
	   SPI parameter config  will be
...
)
{
	...
	pio_spi_write8_read8_blocking( &spi_sd, buff, buff, 1 );
	return (BYTE) *buff;
}
...
)
{
	...
	pio_spi_read8_blocking_with(&spi_sd, 0xFF, b, btr);
}
...
)
{
	...
	pio_spi_write8_blocking(&spi_sd, b, btx);
}
