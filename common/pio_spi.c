...
void __time_critical_func(pio_spi_read8_blocking_with)(const pio_spi_inst_t *spi, char tx_val, uint8_t *dst, size_t len) {
	  /* Added by Domeur for SD Card over SPI support, must be able to write a specific value while reading */
    size_t tx_remain = len, rx_remain = len;
    io_rw_8 *txfifo = (io_rw_8 *) &spi->pio->txf[spi->sm];
    io_rw_8 *rxfifo = (io_rw_8 *) &spi->pio->rxf[spi->sm];
    while (tx_remain || rx_remain) {
